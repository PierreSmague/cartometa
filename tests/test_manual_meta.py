...
# --- Editing an existing meta ----------------------------------------------


def test_editing_rewrites_the_text_fields(paths):
    meta = _create(paths)

    update_meta(
        paths, meta["id"],
        title="Bornes blanches", description="Elles sont blanches.",
        category="landscape", source_url="https://example.org/x",
    )

    stored = read_json_list(paths.manual_metas)[0]
    assert stored["title"] == "Bornes blanches"
    assert stored["description"] == "Elles sont blanches."
    assert stored["category"] == "landscape"
    assert stored["source_url"] == "https://example.org/x"


def test_editing_returns_the_updated_meta(paths):
    """The interface redraws from the return value rather than reloading the queue,
    which would reset a footprint being drawn."""
    meta = _create(paths)

    updated = update_meta(paths, meta["id"], title="Neuf",
                          description="Description neuve.", category="car")

    assert updated["title"] == "Neuf"
    assert updated["id"] == meta["id"]


def test_editing_sets_the_difficulty(paths):
    meta = _create(paths)

    update_meta(paths, meta["id"], title="T", description="D",
                category="car", difficulty="Pro")

    assert read_json_list(paths.manual_metas)[0]["difficulty"] == "Pro"


def test_clearing_the_difficulty_drops_the_field_entirely(paths):
    """"Not rated" is the absence of the field, not an empty string: the build reads
    a missing key, and a meta that carries `""` would publish the same thing while
    reading as a decision that was made."""
    meta = _create(paths, difficulty="Pro")

    update_meta(paths, meta["id"], title="T", description="D",
                category="car", difficulty="")

    assert "difficulty" not in read_json_list(paths.manual_metas)[0]


def test_an_unknown_difficulty_is_refused_on_edit(paths):
    meta = _create(paths)

    with pytest.raises(ManualMetaError, match="Expert"):
        update_meta(paths, meta["id"], title="T", description="D",
                    category="car", difficulty="Expert")


def test_an_unknown_category_is_refused_on_edit(paths):
    meta = _create(paths)

    with pytest.raises(ManualMetaError, match="landscpae"):
        update_meta(paths, meta["id"], title="T", description="D",
                    category="landscpae")


def test_an_empty_title_is_refused_on_edit(paths):
    """Same rule as on creation: the site has nothing to show without a title."""
    meta = _create(paths)

    with pytest.raises(ManualMetaError, match="title"):
        update_meta(paths, meta["id"], title="   ", description="D", category="car")


def test_a_refused_edit_writes_nothing(paths):
    meta = _create(paths)
    ...
    assert read_json_list(paths.manual_metas)[0]["title"] == meta["title"]


def test_editing_leaves_the_fields_it_does_not_own_alone(paths):
    """`id`, `country`, `tier`, `origin`, `extracted_at`, `image`: identity and
    provenance are not editorial content, and the form never shows them."""
    meta = _create(paths)
    intacts = {cle: meta[cle] for cle in
               ("id", "country", "tier", "origin", "extracted_at", "description_origin")}

    update_meta(paths, meta["id"], title="T", description="D", category="car")

    stored = read_json_list(paths.manual_metas)[0]
    assert {cle: stored[cle] for cle in intacts} == intacts


def test_editing_one_meta_leaves_its_neighbours_untouched(paths):
    premier = _create(paths, title="Premier")
    second = _create(paths, title="Second")

    update_meta(paths, second["id"], title="Second modifié",
                description="D", category="car")

    stored = read_json_list(paths.manual_metas)
    assert [m["title"] for m in stored] == ["Premier", "Second modifié"]


def test_a_tagged_meta_cannot_be_edited(paths_with_a_tagged_meta):
    """Its texts live in `data/metas/`, which is gitignored and regenerated by
    `cartometa-import-tagged`: the edit would look saved, then vanish on the next
    import. Refused with the reason, rather than silently lost."""
    with pytest.raises(ManualMetaError, match="data/manual"):
        update_meta(paths_with_a_tagged_meta, "tag-a1b2c3",
                    title="T", description="D", category="car")


def test_an_unknown_meta_cannot_be_edited(paths):
    _create(paths)

    with pytest.raises(ManualMetaError, match="man-0000"):
        update_meta(paths, "man-0000", title="T", description="D", category="car")
