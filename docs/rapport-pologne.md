...
- Métas validées sans retouche à la revue : en attente — la revue humaine (Task 11) n'a pas encore eu lieu ; les 37 métas sont toutes au statut `auto`. À mesurer par la personne qui effectue la revue au clavier (`cartometa-review`), une fois celle-ci terminée, comme `validées sans modification / total revu`.
...
La spec (§6.1) reportait la construction de l'éditeur de sommets jusqu'à connaître le taux de validation sans retouche à la revue. Ce taux ne peut pas encore être établi : la revue humaine n'a pas eu lieu sur ce corpus (37/37 métas au statut `auto`).

Décision : le report est maintenu. L'éditeur de sommets reste hors scope tant que ce taux n'est pas mesuré. Une fois une session de revue réalisée sur les 37 métas PL (via `cartometa-review`), recalculer `validées sans retouche / total revu` et rouvrir la décision sur cette base — pas avant.
...
- Prochaine étape recommandée : mener la revue humaine des 37 métas PL avec `cartometa-review`, pour (a) débloquer les deux chiffres de revue laissés en attente ci-dessus, (b) produire un premier jeu de données réellement publiable via `cartometa-export PL` (sans `--include-auto`), et (c) alimenter la décision sur l'éditeur de sommets avec une vraie mesure.
